...
if(alarm[0] <= 0){
if(alarm[2] >= 0){
    if(sprite_index != spr_mutant_wolf_attack) hsp = 4*sign(image_xscale);
    else hsp = 0
} else {
if(!hit)
...
if(place_meeting(x,y,obj_player)){
    if(alarm[2] <=0 && !attack){
        ...
        dmg.damage = (global.enemy_damage * 2) + 2;
        movespeed = 0;
        sprite_index = spr_mutant_wolf_attack;
        image_speed = 0.35
    }
}
